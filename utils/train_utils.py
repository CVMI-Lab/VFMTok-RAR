...
from evaluations.evaluator import VQGANEvaluator
...
def batch_data_collate(dd):

    assert isinstance(dd[0][0], torch.Tensor)
    samples = torch.stack([sample[0] for sample in dd], dim=0)
    labels = torch.Tensor([sample[1] for sample in dd])
    return {'image': samples, 'class_id': labels}
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        if config.dataset.params.get("pretokenization", ""):
            ...
            if "image" in batch:
                ...
                with torch.no_grad():
                    input_tokens = tokenizer.encode_topk_indices(images, n_index=2)

                conditions = repeat(conditions.unsqueeze(1), 'b k -> (b k j)', j=input_tokens.size(1))
                input_tokens = input_tokens.view(-1, input_tokens.size(2)).contiguous()
        ...
        with accelerator.accumulate([model]):
            ...
            if model_type == "maskgit":
                ...
                # labels = condition.unsqueeze(1).int()
                logits, labels = model(input_tokens.long(), condition.int(), return_labels=True)
