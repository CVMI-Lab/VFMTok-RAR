...
from vfmtok.engine.ema import requires_grad
...
def get_rar_generator(config, ckpt=None):
    
    model_cls = RAR
    generator = model_cls(config)
    if ckpt is None:
        assert osp.exists(config.experiment.generator_checkpoint), f"Please ensure the existence of the checkpoint."
        state_dict = torch.load(config.experiment.generator_checkpoint,'cpu')
    else:
        assert osp.exists(ckpt), f'Please ensure the existence of {ckpt}.'
        # state_dict = torch.load(ckpt, 'cpu')
        state_dict = load_file(ckpt, 'cpu')
...
def sample_fn(generator,
    ...
    generated_image = torch.clamp(generated_image, 0, 255).permute(0, 2, 3, 1).to('cpu', dtype=torch.uint8).numpy()
