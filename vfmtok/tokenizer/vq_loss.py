...
from vfmtok.engine.misc import get_world_size
...
class VQLoss(nn.Module):
    ...
    def forward(self, codebook_loss, inputs, outputs, optimizer_idx, global_step, last_layer=None, 
                logger=None, log_every=100):
        
        reconstructions, latent, dinov2 = outputs
        #* Generator update
        if optimizer_idx == 0:
            #* Reconstruction loss
            ...
            loss = self.rec_weight * rec_loss + \
                ...
                codebook_loss[0] + codebook_loss[1] + codebook_loss[2] + sim_loss.mean()
            
            #* Log losses to waste training time.
