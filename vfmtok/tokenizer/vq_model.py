...
class VQModel(nn.Module):
    ...
    def encode_topk_indices(self, inp, n_index=2):


        slots, latent = self.encode_transformer(inp,)

        queries = self.pre_slots_quant(slots)
        queries = rearrange(queries.unsqueeze(2), 'b h w c -> b c h w')

        _, indices = self.slot_quantize.get_neighboring_codes(queries, k=n_index)

        return indices

    def encode_indices(self, inp):

        _, _, indices = self.encode(inp)
        return indices
    ...
    def decode_tokens(self, codes, tgt_size=256):
...
class VectorQuantizer(nn.Module):
    ...
    def get_neighboring_codes(self, z, k = 2):
        ...
        sorted_encoding_dist, sorted_encoding_index = torch.sort(d, dim=1)
        min_encoding_indices = sorted_encoding_index[:, :k]
        min_encoding_dist = torch.gather(d, 1, min_encoding_indices,)

        min_encoding_index = torch.argmin(d, dim=1)
        min_single_dist = torch.gather(d, 1, min_encoding_index[:, None])
        assert torch.all(min_encoding_dist[:,:1] - min_single_dist) < 1e-6

        z_q = embedding[min_encoding_indices]
        z_hat = embedding[min_encoding_index]
        assert torch.all(z_q[:,0] - z_hat < 1e-6)

        z_q = rearrange(z_q, '(b h w) k c -> b k c h w', b=bs, h=h, w=w, k=k).contiguous()
        min_encoding_indices = rearrange(min_encoding_indices, '(b h w) k -> b k h w', b=bs, h=h, w=w).contiguous()
        return z_q, min_encoding_indices.flatten(-2)
